...
class PeriodeController extends Controller
{
    ...
    {
        $model = new Periode();
        $last = Periode::find()->orderBy(['date_from' => SORT_DESC])->one();
        if ($last) {
            $fr = strtotime($last->date_to) + 24 * 3600;
        } else {
            $fr = strtotime(date('Y-m-01'));
        }
        
        $model->name = date('F, Y', $fr);
        $model->date_from = date('Y-m-d', $fr);
        $model->date_to = date('Y-m-t', $fr);
